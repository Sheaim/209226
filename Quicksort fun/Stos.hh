...
template<typename typ>
void Stos<typ>::operator=(Stos other)
{
...
/*!
 * \brief definicje gettera i settera dla konkretnych elementow tab
 * \param [index] adres komorki do ktorej odnosi sie funkcja
 * \param [value] nowa ustalana wartosc komorki
 */
template<typename typ>
typ * Stos<typ>::get_tab()
{
    return tab;
}
template<typename typ>
typ Stos<typ>::get_tab(int index)
{
    return tab[index];
}
template<typename typ>
bool Stos<typ>::set_tab(typ* other)
{
    tab = other;
    return 0;
}
...
void Stos<typ>::push(typ element)
{
    ...
    delete [] tmp;
    rozmiar=licznik;
    std::cout<<tab[licznik-1]<<std::endl;
}
